...
static void test(const char *observed, size_t expected_count, const char *const expected_pins[]);

Test(Example_tests, fixed_tests) {

    const char* observed = "8";
    size_t expected_count = 5;
    const char *const expected_pins[] = { "0", "5", "7", "8", "9" };
    test(observed, expected_count, expected_pins);

    observed = "11";
    expected_count = 9;
    const char *const expected_pins2[] = { "11", "12", "14", "21", "22", "24", "41", "42", "44" };
    test(observed, expected_count, expected_pins2);
    ...
    test(observed, expected_count, expected_pins3);
}
...
static void test(const char *observed, size_t expected_count, const char *const expected_pins[]) {
    ...
    cr_assert_eq(count, expected_count, "Expected %zu pins, received %zu", expected_count, count);
    qsort(pins, count, sizeof(pins[0]), compare_function);
    for (size_t i = 0; i < count; ++i) {
        if (strcmp(pins[i], expected_pins[i])) {
            cr_assert_fail("%zuth PIN was expected to be '%s', but was '%s'", i, expected_pins[i], pins[i]);
        }
    }
    free_pins(pins);
}